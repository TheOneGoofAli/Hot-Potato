...
-- Properly init keys stuff to be compatible with existing save files
function init_keys(game)

    game.keys = game.keys or 0
    game.keys_per_round = game.keys_per_round or 2
    game.current_round.keys = game.current_round.keys or 0

end


function add_round_eval_keys(config)
    local config = config or {}
    local width = G.round_eval.T.w - 0.51
    local num_dollars = config.keys or 1
    local scale = 0.9
    ...
    delay(0.2)
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                local left_text = {}
                if config.name == 'keys' then
                  table.insert(left_text, {n=G.UIT.T, config={text = config.keys, font = config.font, scale = 0.8*scale, colour = G.C.BLUE, shadow = true, juice = true}})
                  table.insert(left_text, {n=G.UIT.O, config={object = DynaText({string = {" "..localize{type = 'variable', key = 'hotpot_keys_cashout', vars = {G.GAME.keys_per_round or 0}}}, colours = {G.C.UI.TEXT_LIGHT}, shadow = true, pop_in = 0, scale = 0.4*scale, silent = true})}})
                end
      ...
      G.GAME.current_round.keys = G.GAME.current_round.keys + config.keys
      ...
      {n=G.UIT.C, config={align = "cm", padding = 0.05, minw = 1.55, minh = 1.15, colour = temp_col, emboss = 0.05, r = 0.1}, nodes={
        {n=G.UIT.R, config={align = "cm"}, nodes={
          {n=G.UIT.C, config={align = "cm", r = 0.1, minw = 1.25, minh = 1, colour = temp_col2}, nodes={
            {n=G.UIT.O, config={object = DynaText({string = {{ref_table = G.GAME, ref_value = 'keys', prefix = localize('$')}}, maxw = 1.2, colours = {G.C.MONEY}, shadow = true,spacing = 2, bump = true, scale = 2.2*scale}), id = 'keys_text_UI'}}
          }},
        }},
      }},
    }})
